...
// PresetDescriptor is the cross-product descriptor shape returned by
// both the CLI (`dbounce audit-webhook presets list --json`) and
// the MCP tool (`list_audit_webhook_presets`). Field set matches
// the ibounce + kbounce descriptor shape byte-for-byte so an agent
// calling the matching tool on any bouncer sees identical JSON.
...
// for every audit-webhook preset dbounce speaks.
//
// Exposed in the audit package so both the CLI surface and the MCP
// tool can import it without creating a circular dependency.
	...
	return []PresetDescriptor{
		{
			...
			BodyShape:  "NDJSON of OCSF v1.1.0 class 6003 events",
